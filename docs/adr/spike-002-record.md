...
| 2026-07-06 | Container half — harness fix | — | **CORRECTED, RE-RUN PENDING** | Assertion 3 rewritten to create the toy files **inside** the container as `bench` (one `sh -s` heredoc, no host mount, no uid crossing) — the WP3-representative delivery path. Re-run `./scripts/spike-adr002.sh --container` to record the result. The three isolation assertions already passed and do not need re-litigating. |
