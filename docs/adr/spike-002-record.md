...
| 2026-07-07 | WP3b.1 executor acceptance (`scripts/verify-workbench.mjs`, not the spike script) — run 1 | Owner's Docker host (node 22.16.0) | **6/7 — real bug via the actual named-volume path** | ❌ in-container fail-before/pass-after check yielded `before=0, after=0`. Root cause: the ADR-005 **named volume** mounts at `/workbench`, but the image's `WORKDIR /workbench` is **root-owned**, so a fresh volume inherits root ownership and non-root `bench` can't write — the fixture's `printf > add.mjs` failed silently. This is the productionized version of run-1's permission signature: run 1's note assumed the named-volume path was bench-owned; the real executor showed it wasn't. **Genuine ADR-005 finding, and the acceptance caught it.** Fix: `chown /workbench` to bench in `docker/workbench.Dockerfile` (commit e8c520f). |
| 2026-07-07 | WP3b.1 executor acceptance — run 2 (post-fix) | Owner's Docker host (same) | **PASS (7/7)** | `WORKBENCH EXECUTOR PASS`: ensure · idempotent · non-root (uid 10001) · exit-0 propagated · exit-7 propagated · in-container check fail-before/pass-after (`before=1, after=0`) · destroy cleans up. **`DockerCommandExecutor` foundation proven on real Docker; WP3b.3 (`bench.*` tools) cleared to start.** |
| 2026-07-07 | WP3b.5 egress acceptance (`scripts/verify-egress.mjs`) — runs 1–2 | Owner's Docker host | **caught 2 real issues** | ❌ run 1: allowlisted host 502'd — the proxy ran with the `--internal` net as its primary, so no default route/DNS to reach the upstream. Fix: primary = outbound `bridge`, internal net attached second (commit cb5b1b2). ❌ run 2: still 502 on `example.com` — diagnostic showed routing OK (TCP 1.1.1.1 by IP) but the network **sinkholes example.com → 5.6.7.8**; a test-host artifact, not a proxy fault. Fix: probe `one.one.one.one`/env-overridable (commit 855ab80). |
| 2026-07-07 | WP3b.5 egress acceptance — run 3 (post-fixes) | Owner's Docker host | **PASS (6/6)** | `EGRESS PROXY PASS`: secret injected at spawn · HTTP(S)_PROXY → sidecar · allowlisted host reachable through the proxy (`registry.npmjs.org → 200`) · non-allowlisted refused (proxy 403) · direct egress blocked by the `--internal` network · destroy removes workbench + proxy + network. **Egress allowlist + vault secret injection proven on real Docker; WP3b.4 (`bench.delegate`) cleared to start.** |
